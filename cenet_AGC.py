...
class CE_Net_AGC(nn.Module):
    def __init__(self, num_classes=1, num_channels=3):
        super(CE_Net_AGC, self).__init__()
        ...
        """
        for para in self.firstconv.parameters():
            para.requires_grad = False
        for para in self.firstbn.parameters():
            para.requires_grad = False
        for para in self.firstrelu.parameters():
            para.requires_grad = False
        for para in self.firstmaxpool.parameters():
            para.requires_grad = False
        for para in self.encoder1.parameters():
            para.requires_grad = False
        """
        self.AGC0 = AGCnet(64,64)#AGC模块

        self.encoder2 = resnet.layer2
        self.encoder3 = resnet.layer3
        self.encoder4 = resnet.layer4
        """
        for para in self.encoder2.parameters():
            para.requires_grad = False
        for para in self.encoder3.parameters():
            para.requires_grad = False
        for para in self.encoder4.parameters():
            para.requires_grad = False
        """

        self.dblock = DACblock(512)
        self.AGC = AGCnet(512,512)#AGC模块
        self.spp = SPPblock(512)
        """
        for para in self.dblock.parameters():
            para.requires_grad = False
        for para in self.spp.parameters():
            para.requires_grad = False
        """
        self.decoder4 = DecoderBlock(516, filters[2])
        self.decoder3 = DecoderBlock(filters[2], filters[1])
        self.decoder2 = DecoderBlock(filters[1], filters[0])
        #self.AGC1 = AGCnet(filters[0],filters[0])#AGC模块
        self.decoder1 = DecoderBlock(filters[0], filters[0])
        """
        for para in self.decoder4.parameters():
            para.requires_grad = False
        for para in self.decoder3.parameters():
            para.requires_grad = False
        for para in self.decoder2.parameters():
            para.requires_grad = False
        for para in self.decoder1.parameters():
            para.requires_grad = False
        """
        ...
        """
        for para in self.finaldeconv1.parameters():
            para.requires_grad = False

        for para in self.finalconv2.parameters():
            para.requires_grad = False

        for para in self.finalconv3.parameters():
            para.requires_grad = False
