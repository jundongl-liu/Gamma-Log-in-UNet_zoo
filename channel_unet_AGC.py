...
class myChannelUnet_AGC(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(myChannelUnet_AGC, self).__init__()
        filter = [64,128,256,512,1024]
        self.conv1 = DoubleConv(in_ch, filter[0])
        self.pool1 = nn.MaxPool2d(2)
        self.AGC0 = AGCnet(filter[0],filter[0]) #AGC模块
        ...
        self.AGC = AGCnet(filter[4],filter[4]) #AGC模块
